...
void phase_defused() {
    ...
    if (*num_input_strings == 6) {
            ebx = &var_50;
            if (sscanf(0x804b770, "%d %s", &var_54, ebx) == 2) {
                    if (strings_not_equal(ebx, "austinpowers") == 0) {
...
int secret_phase() {
    var_18 = ebx;
    esp = (esp - 0x14) + 0x10;
    ebx = __strtol_internal(read_line(), 0x0, 0xa, 0x0); //en base 10
    if (ebx > 0x3e9) { //1000
            eax = explode_bomb();
    }
    else {
...
do {
        if (var_18[edi] > 6) //numbers should be bellow 6
                return explode_bomb();
        ebx = edi + 1;
        if (ebx <= 5) 
        {
                esi = &var_18;
                do {
                        if (*(esi[edi]) == *(esi[ebx]))         //no duplicate
                                return explode_bomb();
                }
                while (++ebx <= 0x5);
        }
    ...
    do {
            esi = 0x804b26c;
            if (ecx[edi] > 1) {              //pour chaque nb > 1
                    ebx = 0x1;
                    eax = ecx[edi];          //nb
                    esi = 0x804b26c;
                    do {
                            esi = *(esi + 0x8); //esi = esi[1]
                            ebx = ebx ++;
                    } while (ebx < eax);
            }
            var_3C[edi] = esi                //var_3C[edi] = esi
            edi++;
    } while (edi <= 5);
    ...
    do {
        ...
        *(esi + 0x8) = 0;
        ...
        do {
                ...
                eax = *esi;                //arr[0]
                edx = *(esi + 0x8);        // arr[1]    //nombre decroissant
                if (eax < *edx)             
                {
                        eax = explode_bomb();
                        return eax;
